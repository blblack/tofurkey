...
fn autokey_setup(mainkey_path: []const u8) !void {
    ...
    var mainkey: [cw.b2b_KEYBYTES]u8 = undefined;
    defer cw.sodium_memzero(&mainkey);
...
fn set_keys_secure(cfg: *const cfg_s, now: u64, ctr_primary: u64, ctr_backup: u64) !void {
    ...
    defer cw.sodium_memzero(&key_ascii);
    {
        var key_primary: [TFO_KEY_LEN]u8 = undefined;
        var key_backup: [TFO_KEY_LEN]u8 = undefined;
        defer cw.sodium_memzero(&key_primary);
        defer cw.sodium_memzero(&key_backup);
        {
            var key_main: [cw.b2b_KEYBYTES]u8 = undefined;
            defer cw.sodium_memzero(&key_main);
            try safe_read_key(&key_main, cfg.mainkey_path);
            try cw.b2b_derive_from_key(&key_primary, TFO_KEY_LEN, ctr_primary, &kdf_ctx, &key_main);
            try cw.b2b_derive_from_key(&key_backup, TFO_KEY_LEN, ctr_backup, &kdf_ctx, &key_main);
        }
