...
fn block_all_signals() posix.sigset_t {
    ...
    posix.sigprocmask(posix.SIG.SETMASK, &os.linux.filled_sigset, &sigmask_prev);
    return sigmask_prev;
}
...
inline fn safe_read_key(mainkey: *[KDF_KEYBYTES]u8, mainkey_path: []const u8) !void {
    const key_fd = try posix.open(mainkey_path, .{ .ACCMODE = .RDONLY, .CLOEXEC = true }, 0);
...
inline fn safe_write_procfs(keys_ascii: *const [TFO_ASCII_ALLOC]u8, procfs_path: []const u8) !void {
    const procfs_fd = try posix.open(procfs_path, .{ .ACCMODE = .WRONLY, .CLOEXEC = true, .SYNC = true }, 0);
...
inline fn safe_write_autokey(mainkey: *const [KDF_KEYBYTES]u8, autokey_path: []const u8) !void {
    const autokey_fd = try posix.open(autokey_path, .{ .ACCMODE = .WRONLY, .CREAT = true, .TRUNC = true, .CLOEXEC = true, .SYNC = true }, posix.S.IRUSR | posix.S.IWUSR);
...
fn blake2b_kdf(out: *[TFO_KEY_LEN]u8, subkey: u64, key: *const [KDF_KEYBYTES]u8) void {
    ...
    std.mem.writeInt(u64, salt[0..8], subkey, .little);
...
const cfg_s = struct {
    ...
    fn _usage(comptime format: []const u8, args: anytype) noreturn {
        ...
        const usage_fmt =
            ...
            \\
            \\This is tofurkey v1.3.2 (EXPERIMENTAL Zig variant)
