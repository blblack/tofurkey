...
F_NONNULL
static void block_all_signals(sigset_t* const saved_sigs)
{
...
F_NONNULL
static void restore_signals(const sigset_t* const saved_sigs)
{
...
static void sysd_notify_ready(void)
{
    ...
    if (plen > sizeof(sun.sun_path))
        log_fatal("NOTIFY_SOCKET path '%s' exceeds sun_path length of %zu",
    ...
    if (strv < 0)
        log_fatal("Cannot send READY=1 to NOTIFY_SOCKET '%s': %s", spath, strerror(errno));
    if (strv != 7)
        log_fatal("Cannot send READY=1 to NOTIFY_SOCKET '%s' (sent %zi/7 bytes)", spath, strv);
    if (close(fd))
        log_fatal("close() of NOTIFY_SOCKET '%s' failed: %s", spath, strerror(errno));
}
...
static bool safe_read_key(uint8_t mainkey[static restrict crypto_kdf_blake2b_KEYBYTES],
                          const char* const restrict mainkey_path)
{
...
static void safe_write_procfs(char keys_ascii[static restrict TFO_ASCII_ALLOC],
                              const char* const restrict procfs_path)
{
...
static void safe_write_autokey(uint8_t mainkey[static restrict crypto_kdf_blake2b_KEYBYTES],
                               const char* const restrict autokey_path)
{
    const size_t klen = crypto_kdf_blake2b_KEYBYTES; // for brevity below
...
F_NONNULL
static void autokey_setup(const char* const mainkey_path)
{
    log_info("No -k arg given.  Will use a host-local "
             "autokey at '%s'. This does not allow for cross-host synch!", mainkey_path);
...
static struct tc_out timecalc(uint64_t now, uint64_t interval)
{
    ...
    assert(now > interval); // enforced indirectly (cf static time asserts near top)
...
F_NONNULL
static void parse_args(const int argc, char** argv, struct cfg* const cfg_p)
{
...
F_NONNULL
static void cfg_cleanup(struct cfg* const cfg_p)
{
...
F_NONNULL
static void set_keys_secure(const struct cfg* const cfg_p, const uint64_t now,
...
F_NONNULL
static uint64_t set_keys(const struct cfg* const cfg_p, const uint64_t now)
{
...
int main(int argc, char* argv[])
{
    if (!geteuid())
        if (mlockall(MCL_CURRENT | MCL_FUTURE | MCL_ONFAULT))
            log_fatal("mlockall(CURRENT|FUTURE|ONFAULT) failed: %s", strerror(errno));
